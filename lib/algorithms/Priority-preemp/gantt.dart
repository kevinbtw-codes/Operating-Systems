...
void adder(int time1, int time2, List<Process> prsNew, int priority) {
  List<Process> prs;
  prsNew.add(Process((time1), time2,priority));
  assignPid(prsNew);
}
  ...
  Widget build(BuildContext context) {
    ...
    if (prsNew.length >= 1) {
      ...
      for (i = 0; i < prsNew.length; i++) {
        ...
        list_index++) {
          prsadd.add(Process(prsNew[i].at, prsNew[i].bt, prsNew[i].priority));
    ...
    if (prsNew.length >= 1) {
      ...
      while (i < prsadd.length - 1) {
        if (prsadd[i].ct < prsadd[i + 1].start_time) {
          prsadd.add(
              Process(prsadd[i].ct, prsadd[i + 1].start_time - prsadd[i].ct,prsadd[i+1].priority));
